function[net]=BP_TB_L2_groupAdj(x,y,desired_error,Learning_Rate,hidden_layers,plotting, lambda, C)
%***
% Note : the concepts and notations that we used To name our variables are
% taken from the attached PDF file of :
...
% desired_error : is a user parameter that satisfy this stop_training
...
% value Otherwise .
% net: contains the most important characteristics of the network
...
sqNorm = 0;
%calculate square norm for all weights in each layer 
for i=1:Number_W
    W=weights(i).F;     % load weights
    sqNorm = sqNorm + norm(W)^2;
end 

%add the L2 regularisation squared norm of weights * lambda
error = error + lambda .* sqNorm;  %note, we have not added the 1/2m term 

%calculate current instance's group size: sum(output == y)
group_size = sum(output == y);

%calculate group error 
group_error = error ./ group_size + C/sqrt(group_size);
...
input=input2;     % reload the original input to repeat the correction process
                  % of the weights
display(error); %TD: need to know this value 
end
insE=[insE error];% store error history of each instance
display(samples); %TD: to get idea of iteration
end
close(h);


if strcmp(plotting,'yes') %check string equality 
subplot(211)
...
grid
subplot(212)
plot(insE);% plotting errors behavior vs training instances 
xlabel(' instances');
ylabel('error function behavior');
grid
end
